...
#include "ns3/multicast-application.h"
...
main(int argc, char* argv[])
{
    ...
    uint16_t ueNum = 7;
    uint16_t group1size=3;  
    uint16_t group2size=3;
    ...
    */
     ...
     * NR simulation:
     * - EpcHelper, which will setup the co`re network
   ...
   // nrSlHelper->SetUeSlAmcAttribute("AmcModel", EnumValue(NrAmc::ErrorModel));
     ...
     * Server app: multicast-applicaiton with creatingPackets boolean set to false
     */
   ...
   // sidelinkClient.SetConstantRate(DataRate(dataRateBeString), udpPacketSizeBe);
    ...
    //Creating custom two-hop socket and application for sender 
    Ptr<Socket> appSocket=Socket::CreateSocket(ueVoiceContainer.Get(0),UdpSocketFactory::GetTypeId());
   ...
   /* appSocket=Socket::CreateSocket(ueVoiceContainer.Get(6),UdpSocketFactory::GetTypeId());
    ...
    std::cout<<"Forwarding Address: "<<forwardAddress<<" Destination "<<destinationAddress<<std::endl;
    senderApp->Setup(appSocket,forwardAddress,port,destinationAddress,port,1024,1,DataRate(dataRateBeString),false);
 ...
 std::cout<<"node #"<<0<<" with address "<<ueVoiceContainer.Get(0)->GetObject<Ipv4>()->GetAddress(1,0).GetLocal()<<std::endl;
    ...
    path.str("");
//adding trace receptions for node 1 and 2 
    path << "/NodeList/" << ueVoiceContainer.Get(2)->GetId()
         << "/ApplicationList/0/$ns3::PacketSink/Rx";
    Config::ConnectWithoutContext(path.str(), MakeCallback(&ReceivePacket));
    path.str("");
   ...
   /* if (!useIPv6)
    {
        // Set Tx traces (MODIFIED FOR TWO HOP APP)
        for (uint32_t ac = 0; ac < 1; ac++)
        {
...
*/ 
    ...
    /*
     * VERY IMPORTANT: Dco not forget to empty the database cache, which would
